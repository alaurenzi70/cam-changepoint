...
def _save_png_pdf(fig: plt.Figure, save_dir: str, fname_png: str) -> str:
    """Save a figure as PNG (for display) and PDF (vector, for the paper).
    Returns the PNG path; the PDF has the same name with extension .pdf."""
    os.makedirs(save_dir, exist_ok=True)
    p = os.path.join(save_dir, fname_png)
    fig.savefig(p, dpi=DEFAULT_FIG_DPI, bbox_inches="tight")
...
def make_pvalue_hist(dist="Exponential", delta=1.0, n_paths=300, horizon=300,
    ...
    fig.tight_layout()
    return _save_png_pdf(fig, save_dir, "fig_pvalue_hist.png")
